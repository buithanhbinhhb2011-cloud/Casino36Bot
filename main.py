...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"💸 Mỗi lượt mời bạn bè thành công: +{REWARD_PER_REF}đ\n"
...
async def confirm_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ref:
        ...
        if not row or not row[0]:
            ...
            c.execute("UPDATE users SET invited_count = invited_count + 1, balance = balance + ? WHERE user_id=?", (REWARD_PER_REF, ref))
            conn.commit()
        conn.close()
